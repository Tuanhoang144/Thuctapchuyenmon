...
if(isset($_POST['btnExport'])){
	...
	$rowCount = 1;
	$sheet->setCellValue('A'.$rowCount,'product_title');
	$sheet->setCellValue('B'.$rowCount,'product_price');
	$sheet->setCellValue('C'.$rowCount,'qty');

    $order_id = $_GET['order_id'];
	$result = $con->query("SELECT * FROM order_products LEFT JOIN products ON products.product_id = order_products.product_id WHERE order_products.order_id = '$order_id '");
	while($row = mysqli_fetch_array($result)){
		$rowCount++;
		$sheet->setCellValue('A'.$rowCount,$row['product_title']);
		$sheet->setCellValue('B'.$rowCount,$row['product_price']);
		$sheet->setCellValue('C'.$rowCount,$row['qty']);
	}
